...
export const LiveTelemetryView: React.FC = () => {
  ...
  const [activityLog, setActivityLog] = useState<Array<{ id: number; time: string; msg: string; type: 'info' | 'alert' | 'success' }>>([]);
  const logSeq = useRef(0);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [seriesError, setSeriesError] = useState<string | null>(null);
  const [pendingTagDelete, setPendingTagDelete] = useState<{ id: string; name: string } | null>(null);
  ...
  const fetchMachines = useCallback(async () => {
    ...
    try {
      ...
      if (res && res.content && res.content.length > 0) {
        ...
        setSelectedMachineId((current) => current || res.content[0].id);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Skip polling while the tab is hidden; the next visible tick catches up.
      if (document.visibilityState === 'visible') fetchTelemetry(selectedMachineId, false);
    }, 3000);
  ...
  // Handle Tag Deletion (confirmed via ConfirmDialog)
  const handleDeleteTag = (mappingId: string, tagName: string) => {
    if (!selectedMachineId) return;
    setPendingTagDelete({ id: mappingId, name: tagName });
  };

  const confirmDeleteTag = async () => {
    if (!selectedMachineId || !pendingTagDelete) return;
    const { id, name } = pendingTagDelete;
    setIsDeletingTag(true);
    try {
      await telemetryApi.deleteTagMapping(selectedMachineId, id);
      fetchTelemetry(selectedMachineId, false);
      addLogEntry(`Tag '${name}' removed`, 'info');
    } catch (err) {
      addLogEntry(`Tag '${name}' could not be removed: ${getErrorMessage(err)}`, 'alert');
    } finally {
  ...
  const handleSimulateEdgePulse = async () => {
    ...
    try {
      ...
      addLogEntry(`Test packet failed to ingest: ${getErrorMessage(err)}`, 'alert');
    } finally {
  ...
  const addLogEntry = (msg: string, type: 'info' | 'alert' | 'success') => {
    ...
    setActivityLog((prev) => [{ id, time, msg, type }, ...prev.slice(0, 19)]);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-substrate-border pb-4">
        <div>
          <div className="flex items-center gap-2">
            <Radio className="text-terminal-cyan" size={20} aria-hidden="true" />
      ...
      {loadError && (
        <ErrorState
          title="Live telemetry could not be loaded"
          error={{ message: loadError }}
          onRetry={() => (selectedMachineId ? fetchTelemetry(selectedMachineId, true) : fetchMachines())}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-substrate-card border border-substrate-border p-4 flex items-center justify-between">
          <div>
            <div className="text-xs font-mono text-industrial-400 uppercase tracking-wider">Asset Identity</div>
        ...
        <div className="bg-substrate-card border border-substrate-border p-4 flex items-center justify-between">
          <div>
            <div className="text-xs font-mono text-industrial-400 uppercase tracking-wider">Protocol Bridge</div>
          ...
          <div className="px-2 py-1 bg-substrate-card border border-industrial-600 text-xs font-mono font-bold text-terminal-cyan">
            TCP/IP
          </div>
        </div>
        ...
        <div className="bg-substrate-card border border-substrate-border p-4 flex items-center justify-between">
          <div>
            <div className="text-xs font-mono text-industrial-400 uppercase tracking-wider">Gateway Stream Status</div>
            <div className="text-sm font-mono font-bold text-white mt-1 flex items-center gap-2">
              <span className={`w-2 h-2 ${liveData?.connectionStatus === 'ONLINE' ? 'bg-emerald-400 animate-ping' : 'bg-rose-500'}`} />
        ...
        <div className={`border p-4 flex items-center justify-between ${healthColor}`}>
          <div>
            <div className="text-xs font-mono uppercase tracking-wider opacity-80">Health Assessment</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-substrate-card border border-substrate-border p-4 relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-mono font-bold tracking-wider text-industrial-300 flex items-center gap-1.5">
              <Gauge size={14} className="text-terminal-cyan" />
              SPINDLE SPEED
            </span>
            <span className="text-xs font-mono text-industrial-400 uppercase">RPM</span>
          </div>
          ...
          <div className="text-xs font-mono text-industrial-400 mt-1">Rated: 0 — 15,000 RPM</div>
          <div className="w-full bg-industrial-900 h-2 mt-3 overflow-hidden border border-substrate-border">
            <div
        ...
        <div className={`bg-substrate-card border p-4 relative overflow-hidden ${vibLimitExceeded ? 'border-rose-500/70' : 'border-substrate-border'}`}>
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-industrial-400 uppercase">ISO 10816</span>
          </div>
        ...
        <div className="bg-substrate-card border border-substrate-border p-4 relative overflow-hidden">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-industrial-400 uppercase">AMPERES</span>
          </div>
          <div className="text-3xl font-mono font-bold text-white mt-3">
            {liveData?.motorCurrentAmps ? liveData.motorCurrentAmps.toFixed(1) : '0.0'}
            <span className="text-xs text-industrial-400 ml-1">A</span>
          </div>
          <div className="text-xs font-mono text-industrial-400 mt-1">FLA Rating: 45.0 A Max</div>
          <div className="w-full bg-industrial-900 h-2 mt-3 overflow-hidden border border-substrate-border">
            <div
        ...
        <div className={`bg-substrate-card border p-4 relative overflow-hidden ${tempLimitExceeded ? 'border-rose-500/70' : 'border-substrate-border'}`}>
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-industrial-400 uppercase">CELSIUS</span>
          </div>
      ...
      <div className="bg-substrate-card border border-substrate-border p-5">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-substrate-border pb-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Resolution Tag */}
            <span className="px-2 py-0.5 text-xs font-mono font-bold bg-cyan-950/40 text-cyan-400 border border-cyan-500/40">
              RESOLUTION: {seriesData?.bucketResolution ? `${seriesData.bucketResolution.toUpperCase()} ROLLUP` : 'AUTO'}
            </span>

            {/* SLA Compliance Badge */}
            <div className="flex items-center gap-1.5 px-2.5 py-0.5 text-xs font-mono border border-substrate-border bg-industrial-900">
              <span className={`w-1.5 h-1.5 ${queryDurationMs !== null && queryDurationMs < 200 ? 'bg-emerald-400' : 'bg-amber-400'}`} />
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-industrial-900 border border-substrate-border px-2.5 py-1">
              ...
              <span className="text-xs font-mono font-bold text-industrial-300">TAG:</span>
              <select
                value={selectedTag}
            ...
            <button
              ...
              className="w-11 h-11 flex items-center justify-center bg-industrial-800 hover:bg-industrial-700 text-industrial-300 border border-substrate-border"
              aria-label="Refresh Time-Series"
            >
              <RefreshCw size={14} aria-hidden="true" className={isSeriesLoading ? 'animate-spin' : ''} />
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 py-3 border-b border-substrate-border text-xs font-mono">
          <div className="bg-industrial-900/60 p-2.5 border border-substrate-border">
            <span className="text-xs text-industrial-400 block uppercase">LATEST MEAN</span>
            <span className="text-lg font-bold text-white">{seriesStats.latest.toFixed(2)}</span>
          </div>
          <div className="bg-industrial-900/60 p-2.5 border border-substrate-border">
            <span className="text-xs text-industrial-400 block uppercase">INTERVAL MIN</span>
            <span className="text-lg font-bold text-cyan-400">{seriesStats.min.toFixed(2)}</span>
          </div>
          <div className="bg-industrial-900/60 p-2.5 border border-substrate-border">
            <span className="text-xs text-industrial-400 block uppercase">INTERVAL PEAK MAX</span>
            <span className="text-lg font-bold text-rose-400">{seriesStats.max.toFixed(2)}</span>
          </div>
          <div className="bg-industrial-900/60 p-2.5 border border-substrate-border">
            <span className="text-xs text-industrial-400 block uppercase">PERIOD AVERAGE</span>
          ...
          <div className="bg-industrial-900/60 p-2.5 border border-substrate-border col-span-2 sm:col-span-1">
            <span className="text-xs text-industrial-400 block uppercase">BUCKET COUNT</span>
        ...
        <div className="relative mt-4 bg-industrial-950 border border-substrate-border p-3 overflow-hidden">
          ...
          {seriesError && (
            <div className="px-4 pt-4">
              <ErrorState
                ...
                error={{ message: seriesError }}
                onRetry={() => fetchTimeSeries(selectedMachineId, selectedTag, timeRange)}
                isRetrying={isSeriesLoading}
              />
            </div>
          )}
          {plotPoints.length === 0 ? (
            <div className="h-48 flex flex-col items-center justify-center text-center font-mono p-6">
              ...
              <p className="text-xs text-industrial-400 mt-1">Readings appear here once the edge gateway sends data for this tag.</p>
            </div>
          ) : (
            <div className="relative">
              <svg
                role="img"
                aria-label={`${selectedTag} over the last ${timeRange}: min ${seriesStats.min.toFixed(2)}, average ${seriesStats.avg.toFixed(2)}, max ${seriesStats.max.toFixed(2)} across ${seriesStats.count} samples`}
              ...
              {hoveredBucket && (
                <div className="absolute top-2 right-4 bg-industrial-900/95 border border-cyan-500/60 p-2 text-xs font-mono text-white pointer-events-none shadow-lg z-20">
                  <div className="text-xs text-cyan-400 font-bold border-b border-substrate-border pb-1 mb-1">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mt-3 pt-2 border-t border-substrate-border text-xs font-mono text-industrial-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-substrate-card border border-substrate-border p-4">
          <div className="flex items-center justify-between mb-4 border-b border-substrate-border pb-2">
            <div className="flex items-center gap-2">
              <Sliders size={16} className="text-terminal-cyan" />
            ...
            {canManageTags && (
              <button
                onClick={() => setIsTagModalOpen(true)}
                className="text-xs font-mono text-terminal-cyan hover:underline flex items-center gap-1"
              >
          ...
          {tagMappings.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs font-mono">
                <thead>
                  <tr className="border-b border-substrate-border text-xs text-industrial-400 uppercase tracking-wider">
                  ...
                  {tagMappings.map((tag) => (
                    <tr key={tag.id} className="hover:bg-industrial-900/40">
                      ...
                      <td className="py-2.5 px-3">
                        <span className="px-1.5 py-0.5 text-xs bg-industrial-800 text-industrial-300 border border-substrate-border font-bold">
                          {tag.protocol}
                        </span>
                      </td>
                      <td className="py-2.5 px-3 text-industrial-300 font-mono text-xs truncate max-w-xs">
                        {tag.tagAddress}
                      </td>
                      ...
                      {canManageTags && (
                        <td className="py-2.5 px-3 text-right">
                          <button
                            ...
                            className="w-11 h-11 inline-flex items-center justify-center text-industrial-400 hover:text-hazard-red transition-colors"
        ...
        <div className="bg-substrate-card border border-substrate-border p-4 flex flex-col h-full">
          <div className="flex items-center justify-between mb-3 border-b border-substrate-border pb-2">
            ...
            <span className="text-xs font-mono text-industrial-400">EDGE BUFFER</span>
          </div>

          <div className="flex-1 overflow-y-auto space-y-2 pr-1 max-h-[320px]">
            {activityLog.length === 0 ? (
              <div className="text-xs font-mono text-industrial-500 italic p-4 text-center">
              ...
              activityLog.map((entry) => (
                ...
                >
                  <div className="flex items-center justify-between text-xs opacity-75 mb-0.5">
      ...
      >
        <form onSubmit={handleCreateTag} className="space-y-4">
          ...
          <div>
            <label htmlFor="live-telemetry-field-1" className="block text-xs font-mono font-bold text-industrial-300 mb-1">
              CANONICAL TAG NAME *
            </label>
            <input id="live-telemetry-field-1"
              type="text"
              required
              placeholder="e.g. SPINDLE_SPEED, VIBRATION_RMS, HYDRAULIC_PRESSURE"
              value={newTagName}
              onChange={(e) => setNewTagName(e.target.value)}
              className="w-full bg-industrial-900 border border-substrate-border px-3 py-2 text-xs font-mono text-white focus:border-terminal-cyan outline-none uppercase"
            />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="live-telemetry-field-2" className="block text-xs font-mono font-bold text-industrial-300 mb-1">
                PROTOCOL *
              </label>
              <select id="live-telemetry-field-2"
                value={newProtocol}
                onChange={(e) => setNewProtocol(e.target.value as ProtocolType)}
                className="w-full bg-industrial-900 border border-substrate-border px-3 py-2 text-xs font-mono text-white focus:border-terminal-cyan outline-none"
              >
            ...
            <div>
              <label htmlFor="live-telemetry-field-3" className="block text-xs font-mono font-bold text-industrial-300 mb-1">
                UNIT OF MEASURE
              </label>
              <input id="live-telemetry-field-3"
                type="text"
          ...
          <div>
            <label htmlFor="live-telemetry-field-4" className="block text-xs font-mono font-bold text-industrial-300 mb-1">
              CONTROLLER NODE ID / ADDRESS / TOPIC *
            </label>
            <input id="live-telemetry-field-4"
              type="text"
              required
          ...
          <div>
            <label htmlFor="live-telemetry-field-5" className="block text-xs font-mono font-bold text-industrial-300 mb-1">
              CALIBRATION SCALE FACTOR
            </label>
            <input id="live-telemetry-field-5"
              type="number"
              step="0.0001"
              ...
              className="w-full bg-industrial-900 border border-substrate-border px-3 py-2 text-xs font-mono text-white focus:border-terminal-cyan outline-none"
            />
            <span className="text-xs font-mono text-industrial-400 mt-1 block">
      ...
      >
        <div className="space-y-4">
          ...
          <div className="border border-substrate-border divide-y divide-substrate-border text-xs font-mono">
            <div className="p-3 bg-industrial-900 flex items-center justify-between">
              <div>
                ...
                <span className="text-xs text-industrial-400">Micro-second vibration & spindle diagnostic points</span>
              </div>
              <span className="px-2 py-0.5 bg-emerald-950 text-emerald-400 border border-emerald-500/40 text-xs font-bold">
                7 DAYS RETENTION
              </span>
            </div>
            ...
            <div className="p-3 bg-industrial-900 flex items-center justify-between">
              <div>
                ...
                <span className="text-xs text-industrial-400">Aggregated min, max, avg, and sample counts for shift analysis</span>
              </div>
              <span className="px-2 py-0.5 bg-cyan-950 text-cyan-400 border border-cyan-500/40 text-xs font-bold">
                30 DAYS RETENTION
              </span>
            </div>
            ...
            <div className="p-3 bg-industrial-900 flex items-center justify-between">
              <div>
                ...
                <span className="text-xs text-industrial-400">Long-term trending and multi-month operational compliance</span>
              </div>
              <span className="px-2 py-0.5 bg-amber-950 text-amber-400 border border-amber-500/40 text-xs font-bold">
                365 DAYS RETENTION
              </span>
            </div>
          </div>
          ...
          {retentionReport && (
            <div className="p-3 bg-industrial-950 border border-substrate-border text-xs font-mono grid grid-cols-2 gap-2">
      ...
      <ConfirmDialog
        ...
        message={pendingTagDelete ? `Readings for ${pendingTagDelete.name} will stop being collected for this machine.` : ''}
